...
def register_families_tools(mcp: FastMCP):
    ...
    async def gramps_web_families(
        action: str = Field(
            description="Action to perform. One of: 'get_family', 'post_families', 'post_merge_family'"
        ),
        ...
        if ctx:
            await ctx.info(f"Executing gramps_web_families action: {action}")
        import json
