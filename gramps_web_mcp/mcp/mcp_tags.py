...
def register_tags_tools(mcp: FastMCP):
    @mcp.tool(tags={"tags"})
    async def gramps_web_tags(
        action: str = Field(
            description="Action to perform. One of: 'get_tag', 'get_tags'"
        ),
        ...
        """Manage Gramps Web tags operations. CONCEPT:GRMP-001."""
        if ctx:
            await ctx.info(f"Executing gramps_web_tags action: {action}")
        import json
